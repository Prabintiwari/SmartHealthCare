...
const SideBar = () => {
  ...
  return (
    <div className="min-h-[90vh] border-r bg-white dark:bg-slate-800 dark:border-slate-700">
      ...
      {dToken && (
        <ul className="flex flex-col items-center gap-5 mt-5">
          <NavLink to={"/doctor-dashboard"} onClick={()=>{scrollTo(0,0)}} className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? "bg-indigo-100 dark:bg-slate-700 text-indigo-800 dark:text-white border-r-4 border-indigo-700" : ""}`}>
            <FontAwesomeIcon icon={faHome} />
            <p>Dashboard</p>
          </NavLink>
          <NavLink to={"/doctor-appointments"} onClick={()=>{scrollTo(0,0)}} className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? "bg-indigo-100 dark:bg-slate-700 text-indigo-800 dark:text-white border-r-4 border-indigo-700" : ""}`}>
            <FontAwesomeIcon icon={faCalendar} />
            <p>Appointments</p>
          </NavLink>
          <NavLink to={"/doctor-profile"} onClick={()=>{scrollTo(0,0)}} className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? "bg-indigo-100 dark:bg-slate-700 text-indigo-800 dark:text-white border-r-4 border-indigo-700" : ""}`}>
